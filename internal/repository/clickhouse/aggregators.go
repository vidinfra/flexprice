...
func GetAggregator(aggregationType types.AggregationType) events.Aggregator {
	...
	case types.AggregationSumWithCoeff:
		return &SumWithCoeffAggregator{}
...
// SumWithCoeffAggregator implements sum with coefficient aggregation
type SumWithCoeffAggregator struct{}

func (a *SumWithCoeffAggregator) GetQuery(ctx context.Context, params *events.UsageParams) string {
	windowSize := formatWindowSize(params.WindowSize)
	selectClause := ""
	windowClause := ""
	groupByClause := ""
...
func (a *SumWithCoeffAggregator) GetType() types.AggregationType {
	return types.AggregationSumWithCoeff
}
