...
func (s *CustomerServiceSuite) TestListCustomersByFilter() {
	// Prepopulate the repository with customers for testing
	...
	// Create customers in the repository
	...
	testCases := []struct {
		...
		},
		{
			name: "invalid_filter",
			filter: &types.CustomerSearchFilter{
				Limit: lo.ToPtr(int(-1)), // Invalid limit
			},
			expectedError: true,
			expectedCount: 0,
		},
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			resp, err := s.service.ListCustomersByFilter(s.ctx, tc.filter)
			...
			if tc.expectedError {
				...
				s.T().Logf("Test Case: %s", tc.name)
				s.T().Logf("Filter: %+v", tc.filter)
				s.T().Logf("Total Items: %d", len(resp.Items))
				s.T().Logf("Pagination: %+v", resp.Pagination)

				// Verify the number of items
				s.Equal(tc.expectedCount, len(resp.Items), "Number of items does not match expected count")

				// Verify pagination details
				if tc.filter != nil {
					limit := lo.FromPtr(tc.filter.Limit)
					offset := lo.FromPtr(tc.filter.Offset)
					s.NotNil(resp.Pagination, "Pagination should not be nil")
					s.Equal(limit, resp.Pagination.Limit, "Pagination limit does not match")
					s.Equal(offset, resp.Pagination.Offset, "Pagination offset does not match")
				}
			}
		})
	}
