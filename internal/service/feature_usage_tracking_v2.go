...
// featureUsageTrackingV2Service handles feature usage tracking operations for metered events
type FeatureUsageTrackingV2Service interface {
...
type featureUsageTrackingV2Service struct {
	ServiceParams
...
// NewfeatureUsageTrackingV2Service creates a new feature usage tracking service
func NewFeatureUsageTrackingV2Service(
	params ServiceParams,
	eventRepo events.Repository,
	featureUsageRepo events.FeatureUsageV2Repository,
) FeatureUsageTrackingV2Service {
	ev := &featureUsageTrackingV2Service{
...
func (s *featureUsageTrackingV2Service) PublishEvent(ctx context.Context, event *events.Event, isBackfill bool) error {
...
func (s *featureUsageTrackingV2Service) RegisterHandler(router *pubsubRouter.Router, cfg *config.Configuration) {
...
// RegisterHandler registers a handler for the feature usage tracking topic with rate limiting
func (s *featureUsageTrackingV2Service) RegisterHandlerLazy(router *pubsubRouter.Router, cfg *config.Configuration) {
	// Add throttle middleware to this specific handler
...
func (s *featureUsageTrackingV2Service) processMessage(msg *message.Message) error {
...
func (s *featureUsageTrackingV2Service) processEvent(ctx context.Context, event *events.Event) error {
...
func (s *featureUsageTrackingV2Service) generateUniqueHash(event *events.Event, meter *meter.Meter) string {
...
func (s *featureUsageTrackingV2Service) prepareProcessedEvents(ctx context.Context, event *events.Event) ([]*events.FeatureUsage, error) {
...
func (s *featureUsageTrackingV2Service) findMatchingPricesForEvent(
	event *events.Event,
	prices []*price.Price,
...
func (s *featureUsageTrackingV2Service) checkMeterFilters(event *events.Event, filters []meter.Filter) bool {
	if len(filters) == 0 {
...
func (s *featureUsageTrackingV2Service) extractQuantityFromEvent(
	event *events.Event,
	meter *meter.Meter,
...
func (s *featureUsageTrackingV2Service) convertValueToDecimal(val interface{}, event *events.Event, meter *meter.Meter) (decimal.Decimal, string) {
...
func (s *featureUsageTrackingV2Service) convertValueToString(val interface{}) string {
	switch v := val.(type) {
	case string:
		return v
...
func (s *featureUsageTrackingV2Service) GetDetailedUsageAnalytics(ctx context.Context, req *dto.GetUsageAnalyticsRequest) (*dto.GetUsageAnalyticsResponse, error) {
	// 1. Validate request
...
func (s *featureUsageTrackingV2Service) validateAnalyticsRequest(req *dto.GetUsageAnalyticsRequest) error {
	// External customer ID is now optional - if not provided, analytics will be fetched for all customers
	// No validation needed for empty external_customer_id
...
func (s *featureUsageTrackingV2Service) fetchAnalyticsData(ctx context.Context, req *dto.GetUsageAnalyticsRequest) (*AnalyticsData, error) {
	// 1. Fetch customer(s)
	...
	// 2. Aggregate data across all customers
	return s.aggregateAnalyticsAcrossCustomers(ctx, req, customers)
}

// aggregateAnalyticsAcrossCustomers aggregates analytics data across multiple customers
func (s *featureUsageTrackingV2Service) aggregateAnalyticsAcrossCustomers(ctx context.Context, req *dto.GetUsageAnalyticsRequest, customers []*customer.Customer) (*AnalyticsData, error) {
	if len(customers) == 0 {
		return nil, ierr.NewError("no customers found").
			WithHint("No customers available for analytics").
			Mark(ierr.ErrNotFound)
	}

	// Initialize aggregated data structure
	aggregatedData := &AnalyticsData{
		Customer:              nil, // Will be nil for multi-customer analytics
	...
	// Track currencies to ensure consistency
	...
	for _, customer := range customers {
		...
		if err != nil {
			s.Logger.Warnw("failed to fetch subscriptions for customer",
				"customer_id", customer.ID,
				"external_customer_id", customer.ExternalID,
				"error", err,
			)
			continue // Continue with other customers
		}

		// Validate currency consistency
		currency, err := s.validateCurrency(subscriptions)
		if err != nil {
			s.Logger.Warnw("currency validation failed for customer",
				"customer_id", customer.ID,
				"external_customer_id", customer.ExternalID,
				"error", err,
			)
		...
		if err != nil {
			s.Logger.Warnw("failed to fetch analytics for customer",
				"customer_id", customer.ID,
				"external_customer_id", customer.ExternalID,
				"error", err,
			)
		...
		aggregatedData.Subscriptions = append(aggregatedData.Subscriptions, subscriptions...)
		for _, sub := range subscriptions {
			aggregatedData.SubscriptionsMap[sub.ID] = sub
			for _, lineItem := range sub.LineItems {
				aggregatedData.SubscriptionLineItems[lineItem.ID] = lineItem
			}
		}

		// Aggregate analytics
		aggregatedData.Analytics = append(aggregatedData.Analytics, analytics...)
	}

	// Set currency (use the first one found, or empty if none)
	if len(currencies) > 0 {
	...
	// If we have multiple currencies, log a warning
	if len(currencies) > 1 {
		s.Logger.Warnw("multiple currencies found in aggregated analytics",
			"currencies", currencies,
			"using_currency", aggregatedData.Currency,
		)
	}

	// 6. Enrich with metadata if we have analytics data
	if len(aggregatedData.Analytics) > 0 {
		if err := s.enrichWithMetadata(ctx, aggregatedData, req); err != nil {
			s.Logger.Warnw("failed to enrich analytics with metadata",
				"error", err,
				"analytics_count", len(aggregatedData.Analytics),
			)
...
func (s *featureUsageTrackingV2Service) buildAnalyticsResponse(ctx context.Context, data *AnalyticsData, req *dto.GetUsageAnalyticsRequest) (*dto.GetUsageAnalyticsResponse, error) {
...
func (s *featureUsageTrackingV2Service) fetchCustomer(ctx context.Context, externalCustomerID string) (*customer.Customer, error) {
	...
	// If no external customer ID provided, return nil
	// The caller should use fetchCustomers instead for fetching all customers
	return nil, ierr.NewError("external customer ID is required").
		WithHint("Please provide an external customer ID or use fetchCustomers for all customers").
		Mark(ierr.ErrValidation)
}

// fetchCustomers fetches all customers when no external customer ID is provided
func (s *featureUsageTrackingV2Service) fetchCustomers(ctx context.Context, req *dto.GetUsageAnalyticsRequest) ([]*customer.Customer, error) {
...
func (s *featureUsageTrackingV2Service) fetchSubscriptions(ctx context.Context, customerID string) ([]*subscription.Subscription, error) {
	subscriptionService := NewSubscriptionService(s.ServiceParams)
	filter := types.NewSubscriptionFilter()
...
func (s *featureUsageTrackingV2Service) buildMaxBucketFeatures(ctx context.Context, params *events.UsageAnalyticsParams) (map[string]*events.MaxBucketFeatureInfo, error) {
...
func (s *featureUsageTrackingV2Service) fetchAnalytics(ctx context.Context, params *events.UsageAnalyticsParams) ([]*events.DetailedUsageAnalytic, error) {
...
func (s *featureUsageTrackingV2Service) createAnalyticsParams(ctx context.Context, req *dto.GetUsageAnalyticsRequest) *events.UsageAnalyticsParams {
...
func (s *featureUsageTrackingV2Service) validateCurrency(subscriptions []*subscription.Subscription) (string, error) {
	if len(subscriptions) == 0 {
		return "", nil
	}
...
func (s *featureUsageTrackingV2Service) enrichWithMetadata(ctx context.Context, data *AnalyticsData, req *dto.GetUsageAnalyticsRequest) error {
...
func (s *featureUsageTrackingV2Service) extractUniqueFeatureIDs(analytics []*events.DetailedUsageAnalytic) []string {
...
func (s *featureUsageTrackingV2Service) fetchSubscriptionPrices(ctx context.Context, data *AnalyticsData) error {
...
func (s *featureUsageTrackingV2Service) enrichAnalyticsWithMetadata(data *AnalyticsData) {
	for _, item := range data.Analytics {
...
func (s *featureUsageTrackingV2Service) calculateCosts(ctx context.Context, data *AnalyticsData) error {
...
func (s *featureUsageTrackingV2Service) calculateBucketedCost(ctx context.Context, priceService PriceService, item *events.DetailedUsageAnalytic, price *price.Price) {
...
func (s *featureUsageTrackingV2Service) calculateRegularCost(ctx context.Context, priceService PriceService, item *events.DetailedUsageAnalytic, meter *meter.Meter, price *price.Price) {
...
func (s *featureUsageTrackingV2Service) aggregateAnalyticsByGrouping(analytics []*events.DetailedUsageAnalytic, groupBy []string) []*events.DetailedUsageAnalytic {
...
func (s *featureUsageTrackingV2Service) createGroupingKey(item *events.DetailedUsageAnalytic, groupBy []string) string {
...
func (s *featureUsageTrackingV2Service) setGroupingFields(aggregated *events.DetailedUsageAnalytic, item *events.DetailedUsageAnalytic, groupBy []string) {
	for _, group := range groupBy {
		switch group {
		case "feature_id":
...
func (s *featureUsageTrackingV2Service) mergeTimeSeriesPoints(existing []events.UsageAnalyticPoint, new []events.UsageAnalyticPoint) []events.UsageAnalyticPoint {
...
func (s *featureUsageTrackingV2Service) getCorrectUsageValue(item *events.DetailedUsageAnalytic, aggregationType types.AggregationType) decimal.Decimal {
...
func (s *featureUsageTrackingV2Service) getCorrectUsageValueForPoint(point events.UsageAnalyticPoint, aggregationType types.AggregationType) decimal.Decimal {
	switch aggregationType {
	case types.AggregationCountUnique:
...
func (s *featureUsageTrackingV2Service) ReprocessEvents(ctx context.Context, params *events.ReprocessEventsParams) error {
...
func (s *featureUsageTrackingV2Service) isSubscriptionValidForEvent(
...
func (s *featureUsageTrackingV2Service) ToGetUsageAnalyticsResponseDTO(ctx context.Context, data *AnalyticsData, req *dto.GetUsageAnalyticsRequest) (*dto.GetUsageAnalyticsResponse, error) {
...
func (s *featureUsageTrackingV2Service) getTotalUsageForWeightedSumAggregation(
	subscription *subscription.Subscription,
	event *events.Event,
...
func (s *featureUsageTrackingV2Service) fetchPlans(ctx context.Context, data *AnalyticsData) (map[string]*plan.Plan, error) {
...
func (s *featureUsageTrackingV2Service) fetchAddons(ctx context.Context, data *AnalyticsData) (map[string]*addon.Addon, error) {
