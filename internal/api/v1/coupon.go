...
// @Summary Validate coupon for subscription
// @Description Validates if a coupon can be applied to a subscription
// @Tags Coupons
// @Accept json
// @Produce json
// @Param coupon_id path string true "Coupon ID"
...
// @Failure 404 {object} ierr.ErrorResponse
// @Failure 500 {object} ierr.ErrorResponse
// @Router /coupons/{coupon_id}/validate-for-subscription/{subscription_id} [get]
// @Security ApiKeyAuth
func (h *CouponHandler) ValidateCouponForSubscription(c *gin.Context) {
	couponID := c.Param("coupon_id")
	if couponID == "" {
		c.Error(ierr.NewError("coupon ID is required").
			WithHint("Invalid request format").
	...
	err := h.couponService.ValidateCouponForSubscription(c.Request.Context(), couponID, subscriptionID)
	if err != nil {
		c.Error(err)
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Coupon is valid for this subscription"})
}
