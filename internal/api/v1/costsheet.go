...
// @Summary Create a new cost sheet (legacy)
// @Description Create a new cost sheet with the specified configuration (deprecated - use /costsheets instead)
// @Tags CostSheets
// @Accept json
// @Produce json
// @Security ApiKeyAuth
...
func (h *CostsheetHandler) CreateCostSheet(c *gin.Context) {
	...
	// This would need to be implemented based on the legacy requirements
	c.JSON(http.StatusCreated, dto.CostSheetResponse{
		ID:       "legacy-not-implemented",
		MeterID:  req.MeterID,
		PriceID:  req.PriceID,
...
// @Summary Get cost breakdown for margin calculation (legacy)
// @Description Calculate input costs for margin analysis (deprecated)
// @Tags CostSheets
// @Accept json
// @Produce json
// @Security ApiKeyAuth
// @Param request body dto.GetCostBreakdownRequest true "Cost breakdown request"
// @Success 200 {object} dto.CostBreakdownResponse
// @Failure 400 {object} ierr.ErrorResponse
// @Failure 500 {object} ierr.ErrorResponse
// @Router /cost/breakdown [post]
// @Deprecated
func (h *CostsheetHandler) GetInputCostForMargin(c *gin.Context) {
	...
	resp, err := h.service.GetInputCostForMargin(c.Request.Context(), &req)
	if err != nil {
		h.log.Error("Failed to get input cost for margin", "error", err)
		c.Error(err)
		return
	}
...
// @Summary Calculate ROI (legacy)
// @Description Calculate return on investment for a subscription (deprecated)
// @Tags CostSheets
// @Accept json
// @Produce json
// @Security ApiKeyAuth
...
func (h *CostsheetHandler) CalculateROI(c *gin.Context) {
	...
	resp, err := h.service.CalculateROI(c.Request.Context(), &req)
	if err != nil {
