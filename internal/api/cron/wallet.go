...
func (h *WalletCronHandler) CheckAlerts(c *gin.Context) {
	...
	// First get all wallets with alerts enabled to collect tenant IDs from alert configs
	walletsForTenants, err := h.walletService.GetWallets(c.Request.Context(), &types.WalletFilter{
		AlertEnabled: lo.ToPtr(true),
	})
	if err != nil {
		h.logger.Errorw("failed to get wallets with alerts enabled", "error", err)
		c.Error(err)
		return
	}

	// Collect unique tenant IDs from alert configs
	uniqueTenantIDs := make(map[string]struct{})
	for _, w := range walletsForTenants.Items {
		if w.AlertConfig != nil && len(w.AlertConfig.AllowedTenantIDs) > 0 {
			for _, tenantID := range w.AlertConfig.AllowedTenantIDs {
				uniqueTenantIDs[tenantID] = struct{}{}
			}
		}
	}

	// Convert map to slice
	tenantIDs := make([]string, 0, len(uniqueTenantIDs))
	for tenantID := range uniqueTenantIDs {
		tenantIDs = append(tenantIDs, tenantID)
	}

	h.logger.Infow("found unique tenant IDs from alert configs", "count", len(tenantIDs), "tenant_ids", tenantIDs)

	// If no tenant IDs found, nothing to do
	if len(tenantIDs) == 0 {
		h.logger.Infow("no tenant IDs found in alert configs, skipping")
		c.JSON(http.StatusOK, gin.H{"status": "completed", "message": "no tenant IDs found in alert configs"})
		return
	}
